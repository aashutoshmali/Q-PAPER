...
Partial Class SubjectMaster
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        txtcode.Attributes.Add("onkeypress", "return chkcode()")
    ...
    Sub getfirstusers(ByVal i As Integer)
        ...
        If ds.Tables("user").Rows.Count >= 1 Then
            ...
            If Not IsDBNull(ds.Tables("user").Rows(i)(2)) Then
                txtsubject.Text = ds.Tables("user").Rows(i)(2)
            Else

                txtsubject.Text = ""
            End If
            If Not IsDBNull(ds.Tables("user").Rows(i)(3)) Then
                txtcode.Text = ds.Tables("user").Rows(i)(3)
            Else
    ...
    Sub getsubject()
        ...
        cmd.CommandText = "SELECT subjectmaster.* FROM subjectmaster order by keycode"
    ...
    Protected Sub btndeleteuser_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        cmd = New OleDbCommand
        cmd.CommandText = "delete from subjectmaster where keycode = " & hdnUserKey.Value & ";"
        ...
        Try
            ...
            path.Open()
            cmd.ExecuteNonQuery()
            Dim str As String = "<script language='javascript'>alert('Subject Deleted Successfully!');</script>"
    ...
    Protected Sub btnUpdateuser_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        cmd = New OleDbCommand
        cmd.CommandText = "update subjectmaster set Department='" & ddldept.SelectedItem.Text.Trim & "',subject='" & txtsubject.Text.Trim & "',subjectcode='" & txtcode.Text.Trim & "' where keycode=" & hdnUserKey.Value & ";"
        ...
        Try
            ...
            path.Open()
            cmd.ExecuteNonQuery()
            Dim str As String = "<script language='javascript'>alert('Subject Updated Successfully!');</script>"
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "msg", str, False)
        Catch ex As Exception
            ex.InnerException.ToString()
        Finally
    ...
    Protected Sub btnSaveuser_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim row() As DataRow
        row = ds.Tables("user").Select("subject='" & txtsubject.Text & "'")
        If row.Length <> 0 Then
            If (row(0)(0).ToString.ToLower = txtsubject.Text.ToLower) Then
                Dim str As String = "<script language='javascript'>alert('Subject Already Exists!');</script>"
                ScriptManager.RegisterStartupScript(Me, Me.GetType(), "UserExist", str, False)
                txtsubject.Text = ""
                ScriptManager1.SetFocus(txtsubject)
        ...
        cmd = New OleDbCommand
        cmd.CommandText = "insert into subjectmaster (department,subject,subjectcode) values('" & ddldept.SelectedItem.Text & "','" & txtsubject.Text.Trim & "','" & txtcode.Text.Trim & "');"
        cmd.Connection = path
        ...
        Try
            ...
            Dim str As String = "<script language='javascript'>alert('Subject Added Successfully!');</script>"
    ...
    Sub bindGrid(ByVal sortExp As String)
        ...
        Dim fillGrid As New OleDbDataAdapter("SELECT subjectmaster.* FROM subjectmaster order by keycode", path)
    ...
    Protected Sub gvFind_DeleteCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs)
        cmd = New OleDbCommand
        cmd.CommandText = "delete form subjectmaster where keycode=" & hdnUserKey.Value & ""
